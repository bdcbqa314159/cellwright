...
Evaluator::Evaluator(Sheet& sheet, const FunctionRegistry& registry)
    : sheet_(sheet), registry_(registry) {}
...
std::vector<CellValue> Evaluator::expand_range(const RangeNode& range) {
    ...
    int32_t c1 = std::min(range.from.col, range.to.col);
    int32_t c2 = std::max(range.from.col, range.to.col);
    ...
    for (int32_t c = c1; c <= c2; ++c) {
        for (int32_t r = r1; r <= r2; ++r) {
            result.push_back(sheet_.get_value({c, r}));
        }
    }
    return result;
}
